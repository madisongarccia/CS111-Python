...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.amount <self.cost:
            return f'You must add ${self.cost-self.amount} more funds.'
        self.stock -=1
        ...
        if self.amount==self.cost:
            ...
            response = f'Here is your {self.name} and ${self.amount-self.cost} change.'
            self.amount = 0
            return response

    def add_funds(self, moneyAdded):
        if self.stock == 0:
            return f'Nothing left to vend. Please restock. Here is your ${moneyAdded}.'
        else:
            self.amount+=moneyAdded
...
class Coin:
    ...
    def worth(self):
            amountAdded = max((Mint.present_year-self.year)-50,0)
            cents = Nickel.cents if isinstance(self,Nickel) else Dime.cents
            return cents + amountAdded
...
def store_digits(n):
    ...
    """
    temporary_link = Link.empty
    while n!=0:
        temporary_link = Link(n%10,temporary_link)
        n//=10
