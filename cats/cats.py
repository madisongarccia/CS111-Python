...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def reader(paragraphs):
        lowPara = lower(paragraphs)
        noPunc = remove_punctuation(lowPara)
        finalList = split(noPunc)
        for t in topic:
            for p in finalList:
                if t == p:
                    return True
...
def accuracy(typed, reference):
    ...
    for i in range(len(reference_words)):
        ...
        if i+1 == len(typed_words):
            break
    return 100.0*(counter/len(typed_words))
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    differenceTracker=[diff_function(typed_word,word,limit) for word in valid_words]
    if min(differenceTracker)>limit:
        return typed_word
    else:
        differenceTracker.index(min(differenceTracker))
        return valid_words[differenceTracker.index(min(differenceTracker))]
...
def minimum_mewtations(start, goal, limit):
    ...
    if start[0] == goal[0]:
        ...
        add = minimum_mewtations(start,goal[1:],limit) # Fill in these lines
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    progress = 0
    for element in range(len(sofar)):
        if sofar[element] == prompt[element]:
            progress+=1
        else:
            break
    report = {'id': user_id, 'progress': progress/len(prompt)}
    upload(report)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i])-1):
            WordTimeList+=[times_per_player[i][j+1]-times_per_player[i][j]]
        PlayerTimeList+=[WordTimeList]
    return match(words,PlayerTimeList)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fastestL = [[] for row in player_indices]
    for i in word_indices:
        low = 0
        for j in player_indices:
            if get_times(match)[j][i] < get_times(match)[low][i]:
                low = j
        fastestL[low].append(get_words(match)[i])
    return fastestL
    # END PROBLEM 10
